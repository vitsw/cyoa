...
<html class="no-js" lang="">
    <head>
		...
		<style> html, body {min-height: 100%; height: 100%;} </style>
    </head>
	<body class="font-sans">
		<div class="min-h-full flex justify-center items-center">
			<div class="max-w-md rounded overflow-hidden shadow-md bg-grey-lightest px-8 relative">
				<div class="absolute" style="top: 16px; right: 15px;">
					<svg version="1.1" id="レイヤー_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
						y="0px" width="50px" height="50px" viewBox="0 0 401.98 559.472" enable-background="new 0 0 401.98 559.472"
						xml:space="preserve">
				...
				<div class="story-header font-serif py-8">
					<h1> {{ .Title }} </h1>
				<div>
				<div class="story-body pt-3">
					{{ range .Paragraphs }}
						<p class="py-2 text-justify"> {{ . }} </p>
					{{ end}}
				</div>
				<hr class="border border-grey-lighter my-4"/>
				<div class="story-options">
					<ul class="list-reset mt-6">
						{{ range .Options }}
							<li class="mt-2"> <a href="/{{ .Arc }}"> {{ .Text }}</a> </li>
						{{ else }}
							<p> The END </p>
						{{ end}}
					</ul>
...
// Story represents a set of chapters
...
// Option to go futher reading
type Option struct {
	Text string `json:"text"`
	Arc  string `json:"arc"`
}

// JSONStory intialize new book with stories from json file
...
// NewHandler create an http handler for stories
func NewHandler(s Story) http.Handler {	
	return handler{s, template.Must(template.New("").Parse(pageTemplate))}
}

// NewHandlerWithTemplate allows to specify custom template
func NewHandlerWithTemplate(s Story, tplFile string) http.Handler {
	return handler{s, template.Must(template.ParseFiles(tplFile))}
}
